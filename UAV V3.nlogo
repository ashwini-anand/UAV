      ...
      ask uav 0[
        ...
        while[abs (xcor - (item (B_FM-next-waypoint - 1) B_FM-path-xcor-list)) > 0.1 or abs (ycor - (item (B_FM-next-waypoint - 1) B_FM-path-ycor-list)) > 0.1][
          ...
          ask payload 12[
            ...
            [
              ; if communicationLink failed and control is entering here for first time (checked using B_CL-down belief) then raise the UAV
              if(B_CL-down = false)[
                ask flightManager 9[
              ...
              ;check if even after raising to high altitude communication was not regained
              ifelse(B_CL-down)[
...
to raiseUp-and-find-commLink
  ifelse(altitude <= 50)[
    ...
    ifelse(communicationLink-failure)[
  ...
  ; if UAV is flying at altitude 50m (its limiting altitude) then communication link can not be regained and hence return
  [
    show "UAV already flying at peak altitude, can not raise further. UAV is returning"
    set B_CL-down true
  ]
